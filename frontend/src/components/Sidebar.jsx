...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 pr-3 rounded-r-xl overflow-y-auto text-white ${selecteduser?"max-mid-hidden":''}`}>
    <div className='pb-5'>
  ...
  {showMenu && (
    ...
    <div className='flex flex-col'>
      {filteredUsers.map((user,index)=>(
        <div key={index} onClick={()=>{setselecteduser(user);setUnseenMessages(prev=>({...prev,[user._id]:null}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm${selecteduser?._id===user._id && `bg-[#282142]/50`}`}>
